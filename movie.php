...
{
    $data = [];
    $stmt = $conn->prepare("SELECT title, image_url, description, moods, trailer_url FROM movies WHERE category = ? AND is_active = 1");
...
$heroImgs = getMoviesByCategory($conn, 'hero');
...
<body>
    ...
    <script>
        const heroImgs = <?php echo json_encode($heroImgs, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE); ?>;
        const moviesList = <?php echo json_encode($moviesList, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE); ?>;
